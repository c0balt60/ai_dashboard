...
| Agent Screen (chat) | `/agent/:id?chat=&project=` (full screen) | Opens `chat`, else the agent's latest chat in `project` (a draft that is created on the first prompt if there is none), else its latest chat. Centered title with agent name and "project · chat"; tapping it opens the chats sheet (General plus one section per project, new chat buttons, switch agent). Status pill, collapsible "Currently working on" banner, greeting with suggestion pills while empty, chat bubbles, typing indicator, quick-prompt chips, composer card (assign button, send); new/rename/clear/delete chat, assign to folder/task, stop |
...
- The UI never touches a backend class directly.
  - It watches providers in `backend_providers.dart` (`agentsProvider`, `projectsProvider`, `tasksProvider`, `chatsProvider`, `messagesProvider(chatId)`, plus derived ones like `agentProvider(id)`, `latestChatProvider`, `taskForTodoProvider`, `tasksByStateProvider` and `dashboardStatsProvider`).
...
- **Chats:** every agent has a default "General" `AgentChat` whose id is the agent id; other chats belong to one project. Messages, prompts and CLI sessions are per chat. `Agent.projectIds` lists every project an agent belongs to (`setAgentProjects`); `projectId`/`workingDir` are where it works right now, and a prompt in a project chat moves it there.
- **Tasks from to-dos:** `AgentTask.description` holds the notes and `AgentTask.brief` is the title plus notes the agent receives. A `TodoLink` makes the backend tick the to-do off when the task completes and untick it if the task is reopened.
